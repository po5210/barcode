class BaselocsController < DomainResourcesController
  ...
  #
  # GET /domains/:domain_id/baselocs.json
  #
  def index
    args = params["_q"]
  ...
  #
  # GET /domains/:domain_id/baselocs/:id/show.json
  #  
  def show
    tr_cd = params[:id]
  ...
  #
  # POST /domains/:domain_id/baselocs/create.json
  #  
  def create
    conn = ActiveRecord::Base.connection()
    result = nil
    check_sql = "SELECT COUNT(*) CNT FROM BASELOC WHERE TR_CD = #{conn.quote(params[:baseloc_cd])}"
    ...
    if(count.to_i == 0)
      insert_sql = "INSERT INTO BASELOC (
        ...
        #{conn.quote(current_user.id)}, TO_CHAR(SYSDATE, 'YYYYMMDDHH24MISS'), 
        #{conn.quote(current_user.id)}, TO_CHAR(SYSDATE, 'YYYYMMDDHH24MISS')
      )"
      ...
      result = {:success => false, :msg => "BASELOC_CD (#{params[:baseloc_cd]}) already exist"}
    end
  ...
  #
  # GET /domains/:domain_id/baselocs/:id/update.json
  #  
  def update
    conn = ActiveRecord::Base.connection()
    sql = "UPDATE 
  ...
  #
  # DELETE /domains/:domain_id/baselocs/:id/destroy.json
  #
  def destroy
    baseloc_cd = params[:id]
    conn = ActiveRecord::Base.connection()
    
    begin
      conn.execute("DELETE FROM BASELOC WHERE BASELOC_CD = #{conn.quote(baseloc_cd)}")
    rescue Exception => e
      raise Hatio::Exception::InvalidRequest, "ERROR : #{e.to_s}"
    end
