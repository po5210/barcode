class BarMatoutsController < DomainResourcesController
  ...
  def index
    ...
    begin
      ...
      master_sql = "SELECT  '' AS CHK
                    ...
                    , A.WHI_SQ
              FROM BAR_MATOUT A                    
             WHERE A.WHO_DT = ARG_WHO_DT
               AND A.BASELOC_CD = #{params[:baseloc_cd]}
               AND A.LOC_CD = #{params[:loc_cd]}
               AND A.WHO_FG = #{params[:who_fg]}
  ...
  #
  # GET /domains/:domain_id/invoices/:id/matout_info.json
  ...
  def matout_info
    ...
    lot_qty = params[:lot_qty]
    lot_no = params[:lot_no]
    
    conn = ActiveRecord::Base.connection()
    sql = "SELECT LIVE_YN FROM BAR_BARMST WHERE BARCODE = #{item_cd}"
    collection = conn.select_all(sql)
    barmst_datas = collection.empty? ? nil : collection[0]
    
    if(barmst_datas && barmst_datas.size > 0)
      barmst_data = part_info_datas[0]
      live_yn = "#{barmst_data['live_yn']}"
    
      debug_print "live_yn[{live_yn}]"
